...
public class ReceiptDemo extends ConnectionScreen {
    ...
    private CheckBox printAsManyJobs;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        printAsManyJobs = new CheckBox(this);
        printAsManyJobs.setText("Print As many Jobs");

        LinearLayout connection_screen_layout = (LinearLayout) findViewById(R.id.connection_screen_layout);
        int index = connection_screen_layout.indexOfChild(discoverPrintersButton);

        connection_screen_layout.addView(printAsManyJobs, index + 1);

    }

    @Override
    public void performTest() {
        if (printAsManyJobs.isChecked()) {
            executeTest(true);
        } else {
            executeTest(false);
        }
